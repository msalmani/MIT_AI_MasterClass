---
title: "ModelingAndPerformanceEvaluations"
...
varImpPlot(RandomForest_Fit, sort=TRUE, cex =0.5, n.var = 10, main = "Random Forest Model - Variables Importance")
...
* binary variables espressed in terms of "yes"/"no" should be converted to a numeric binary
* categorical variables (e.g. states, zip codes, etc.) should be transformed in binary variables, where each variables is a binary representation of one of the categories
...
The five neural networks successfully trained (with 2,3, 4 and 5 hiddens nodes, respectively) are evaluated against the test data set. The only network showing performance close to other models is the the 5-node network. 
...
The _Random Forest_ model has shown the best performance - as an individual model - when evaluated against the test data set. The _Neural Network_ performance could be further improved by either fine tuning the training parameters or modifying the network structure.
